...
function generateRefreshToken(user) {
  return jwt.sign({ user_id: user.user_id, username: user.username,email:user.email }, Refresh_secret , { expiresIn: '7d' });
}
...
const loginUser = asyncErrorHandler(async (req, res) => {
  ...
  const user = await db('users').where({ email }).orWhere({ username: email }).first();
  if (!user) {
